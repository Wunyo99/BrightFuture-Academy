        ...
        <>
            ...
            <section className="section-padding">
                <div className="section-heading">
                    <p className="uppercase text-sm font-medium text-yellow-600 tracking-widest mb-2">Our Team</p>
                    <h1 className="section-title">Experienced Educators</h1>
                    <p className="section-description">Our qualified and passionate staff members are committed to student success</p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-7">
                    {staffMembers.map((staffMember, index) => (
                        <div key={index} className="bg-red-100 shadow-xl rounded-2xl overflow-hidden">
                            <img src={staffMember.img} alt={staffMember.position} className="h-60 w-full object-cover hover:scale-105 duration-500" />
                            <div className="px-4 py-5 space-y-1.5">
                                <h1 className="text-center text-2xl font-medium">{staffMember.name}</h1>
                                <p className="text-center text-red-800 font-medium">{staffMember.position}</p>
                                <p className="text-center text-gray-700 text-sm font-medium">{staffMember.departments}</p>
                                <p>{staffMember.bio}</p>
                            </div>
                        </div>
                    ))}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-7">
                    {qualities.map((quality, idx) => (
                        <div key={idx} className="p-5">
                            <div className={`w-18 h-18 rounded-full flex items-center justify-center mb-5 mx-auto ${quality.bg}`}>
                                <Award className={`${quality.color}`} size={30} />
                            </div>
                            <div className="text-center">
                                <h1 className={`text-3xl md:text-4xl font font-bold mb-2 ${quality.color}`}>{quality.no}</h1>
            ...
            <section className="section-padding bg-gray-900">
                <div className="section-heading">
                    ...
                    <p className="text-white text-lg">We're always looking for passionate educators to join our community</p>
